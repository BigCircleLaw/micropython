...
mp_obj_t frame_hub_distribute(mp_obj_t self_in, mp_obj_t data ) { 	
    
    // frame_hub_content_t *self=MP_OBJ_TO_PTR(self_in);    //从第一个参数里面取出对象的指针

	return mp_const_none;  //返回计算的结果
}
STATIC MP_DEFINE_CONST_FUN_OBJ_2(frame_hub_distribute_obj, frame_hub_distribute);

//定义type的locals_dict_type
STATIC const mp_rom_map_elem_t frame_hub_locals_dict_table[] = {
    {MP_OBJ_NEW_QSTR(MP_QSTR_distribute), MP_ROM_PTR(&frame_hub_distribute_obj)},
};
//这个定义字典的宏定义
